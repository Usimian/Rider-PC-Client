...
def make_env(domain_rand=True, frame_stack=2):
    return lambda: RiderDiffDriveEnv(add_noise=True, domain_rand=domain_rand,
                                     frame_stack=frame_stack, mirror_aug=True)


def evaluate(model, vn, fs, n=12):
    """Deterministic, noise/DR off: survival + command-tracking error."""
    env = RiderDiffDriveEnv(add_noise=False, domain_rand=False, frame_stack=fs)
    stood = 0; ferr = []; yerr = []
    for s in range(n):
    ...
    print(f"  STOOD {stood}/{n} | fwd-track err {np.mean(ferr):.3f} m/s | "
          f"yaw-track err {np.mean(yerr):.2f} rad/s")
...
if __name__ == "__main__":
    ...
    fs = args.frame_stack
    venv = make_vec_env(make_env(not args.no_dr, fs), n_envs=args.nenv)
    ...
    model = PPO("MlpPolicy", venv, verbose=1, device="cpu", n_steps=1024, batch_size=2048,
                ...
                tensorboard_log="./tb")
    print(f"training diff-drive PPO: {args.steps} steps, {args.nenv} envs, DR={not args.no_dr}, FS={fs}")
